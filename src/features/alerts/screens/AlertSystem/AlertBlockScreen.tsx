...
export default function AlertBlockScreen() {
  ...
  const handleProceed = async () => {
    try {
      ...
      const { languageId } = await resolveLanguageSelection({
        userData: user?.data,
      ...
      if (!languageId) {
        Alert.alert(
          t('community.mobile.createPost.validationTitle', 'Validation'),
          t('community.mobile.createPost.languageRequired', 'Please select a language.'),
        );
        return;
      }
      ...
      await createSOSAlert({
        ...
        blockIds: isEntireBuilding ? [] : user.data.blockId ? [user.data.blockId] : [],
