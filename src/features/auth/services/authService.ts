...
import { resolveLanguageSelection } from '../../language/services/languageService';
...
export const fetchUserProfile = async () => {
  ...
  const resolvedLanguage = await resolveLanguageSelection({
    userData: json?.data,
    syncRemote: true,
  });

  if (json?.data) {
    if (resolvedLanguage.languageId && !json.data.languageId) {
      json.data.languageId = resolvedLanguage.languageId;
    }
    if (resolvedLanguage.languageCode && !json.data.languageCode) {
      json.data.languageCode = resolvedLanguage.languageCode;
    }
  }
