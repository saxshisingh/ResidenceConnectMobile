...
const normalizeImageUrl = (path?: string) => {
  ...
  if (['null', 'undefined', 'n/a', 'na'].includes(p.toLowerCase())) {
    return '';
  }
...
const PhoneIcon = ({ color }: { color: string }) => (
  <Svg width={16} height={16} viewBox="0 0 24 24" fill="none">
    <Path
      d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 015 12.84a19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"
      stroke={color}
      strokeWidth={2}
      strokeLinecap="round"
...
const PetIcon = ({ color }: { color: string }) => (
  <Svg width={16} height={16} viewBox="0 0 24 24" fill="none">
    ...
    <Path
      d="M12 18c2.5 0 4.5-1.5 5.5-3.5.5-1-.5-2-1.5-2h-8c-1 0-2 1-1.5 2 1 2 3 3.5 5.5 3.5z"
      fill={color}
    />
  </Svg>
);
...
const GasIcon = ({ color }: { color: string }) => (
  <Svg width={18} height={18} viewBox="0 0 24 24" fill="none">
    <Path
      d="M12 2c1 3-1 6-1 6s4 2 4 6c0 3-2 6-5 6s-5-3-5-6c0-2 1-4 2-5-1.5.5-3 2-3 5 0 4 3 7 6 7s6-3 6-7c0-5-4-8-4-12z"
      fill={color}
      strokeWidth={1}
    />
  </Svg>
...
const BuildingIcon = ({ color }: { color: string }) => (
  <Svg width={16} height={16} viewBox="0 0 24 24" fill="none">
    <Path
      d="M3 21h18M5 21V7l8-4v18M19 21V11l-6-2"
      stroke={color}
      strokeWidth={2}
      strokeLinecap="round"
      strokeLinejoin="round"
...
export default function ProfileScreen({ navigation }: any) {
  ...
  const [profileImageLoadFailed, setProfileImageLoadFailed] =
  ...
  const parkingSlots = useSelector(
    (state: any) => state.parking?.slots ?? [],
  );

  const vehicles = useSelector(
    (state: any) => state.vehicles?.vehicles ?? [],
  );

  const familyMembers = useSelector(
    (state: any) => state.familyMembers?.items ?? [],
  );
   ...
   *
   * because the current Redux user may be stored directly as:
   *
   *   state.auth.user = { userId, firstName, ... }
   *
   * user?.data would therefore be undefined forever.
   */
  if (loading && !userData) {
    return (
      <View
  ...
  const profileImage = normalizeImageUrl(profilePhoto || photo);

  const showPhoto = Boolean(
    profileImage && !profileImageLoadFailed,
  );
  ...
  const menuItems = [
    ...
    },
    {
      id: 'docs',
      label: t(
        'profile.mobile.uploadDocuments',
        'Documents',
      ),
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.heroBadge}>
                ...
                <Text style={styles.heroBadgeText}>
                  {apartmentUnit} · {blockName} ·{' '}
                  {t('profile.mobile.floor', 'Floor')}{' '}
                  {floorNumber}
                </Text>
              </View>
            </View>
        ...
        >
          ...
          >
            ...
            >
              {t('profile.mobile.pet', 'Pet')}:{' '}
              {petOwnership
                ? t('common.values.yes', 'Yes')
                : t('common.values.no', 'No')}
            </Text>
          </View>
        </View>
        ...
        >
          ...
          <Text style={styles.logoutText}>
            {t('profile.mobile.logout', 'Log Out')}
          </Text>
